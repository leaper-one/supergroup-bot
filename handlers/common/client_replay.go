...
func updateClientWelcome(ctx context.Context, clientID, welcome string) error {
	_, err := session.DB(ctx).Exec(ctx, `UPDATE client_replay SET welcome=$2 WHERE client_id=$1`, clientID, welcome)
	return err
}
...
func UpdateClientReplay(ctx context.Context, c *ClientReplay) error {
	...
	_, err := session.DB(ctx).Exec(ctx, query, c.ClientID, c.JoinMsg, c.Welcome, time.Now())
	return err
}
