...
WHERE status=1`, func(rows pgx.Rows) error {
		...
		tools.Println("select transfer_pendding error", err)
		return
	}
	for _, t := range ts {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "20117") {
				...
				SendMonitorGroupMsg(ctx, fmt.Sprintf("转账失败！请及时充值！%s (%s)\n\n 5分钟后重启转账队列...", a.Symbol, t.Memo))
				SendMonitorGroupMsg(ctx, "mixin://transfer/"+client.ClientID)
				time.Sleep(5 * time.Minute)
			}
			continue
		}
		if err := addSnapshot(ctx, t.ClientID, s); err != nil {
			tools.Println("add snapshot error", err)
			continue
		}
		if err := updateTransferToSuccess(_ctx, t.TraceID); err != nil {
			tools.Println("update transfer_pendding error", err)
			continue
		}
	}
}
...
func addSnapshot(ctx context.Context, clientID string, s *mixin.Snapshot) error {
	...
	_, err := session.DB(ctx).Exec(ctx, query, s.SnapshotID, clientID, s.TraceID, s.OpponentID, s.AssetID, s.Amount.String(), s.Memo, s.CreatedAt)
	return err
}
...
func createTransferPending(ctx context.Context, client_id, traceID, assetID, opponentID, memo string, amount decimal.Decimal) error {
	...
	_, err := session.DB(ctx).Exec(ctx, query, client_id, traceID, assetID, opponentID, amount.String(), memo, TransferStatusPending, time.Now())
	return err
}

func updateTransferToSuccess(ctx context.Context, traceID string) error {
	_, err := session.DB(ctx).Exec(ctx, `UPDATE transfer_pendding SET status = 2 WHERE trace_id = $1`, traceID)
	return err
}
