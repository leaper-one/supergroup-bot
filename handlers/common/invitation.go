...
func CreateInvitation(ctx context.Context, userID, clientID, inviterID string) (string, error) {
	...
	_, err := session.DB(ctx).Exec(ctx, query, userID, inviterID, clientID, inviteCode)
	if err != nil {
		return "", err
	}
...
func handleUserInvite(inviteCode, clientID, userID string) {
	...
	if _, err := session.DB(_ctx).Exec(_ctx, q, userID, inviterID, "0"); err != nil {
