...
func UpdateLive(ctx context.Context, u *ClientUser, l *Live) error {
	...
	_, err := session.DB(ctx).Exec(ctx, query, l.LiveID, u.ClientID, l.ImgURL, l.Category, l.Title, l.Description, LiveStatusBefore)
	return err
}
...
func startLive(ctx context.Context, l *Live) error {
	...
	if _, err := session.DB(ctx).Exec(ctx, query, l.LiveID, time.Now()); err != nil {
		return err
	}
...
func stopLive(ctx context.Context, l *Live) error {
	...
	if err := session.DB(ctx).QueryRow(ctx, `SELECT start_at FROM live_data WHERE live_id=$1`, l.LiveID).Scan(&startAt); err != nil {
		return err
	}
	endAt := time.Now()
	if l.Category == LiveCategoryAudioAndImage {
		session.DB(ctx).Exec(ctx, `UPDATE live_replay SET live_id=$1 WHERE client_id=$2 AND created_at>$3 AND created_at<$4`, l.LiveID, l.ClientID, startAt, endAt)
	}
	go func() {
...
func updateLiveStatusByID(ctx context.Context, liveID string, status int) error {
	_, err := session.DB(ctx).Exec(ctx, `UPDATE lives SET status=$2 WHERE live_id=$1`, liveID, status)
	return err
}
...
func handleStatistics(ctx context.Context, l *Live, startAt, endAt time.Time) error {
	...
	if err := session.DB(ctx).QueryRow(ctx, `SELECT COUNT(1) FROM client_users WHERE client_id=$1 AND read_at > $2`, l.ClientID, startAt).Scan(&readCount); err != nil {
		return err
	}
	if err := session.DB(ctx).QueryRow(ctx, `SELECT COUNT(1) FROM client_users WHERE client_id=$1 AND deliver_at > $2`, l.ClientID, startAt).Scan(&deliverCount); err != nil {
		return err
	}
	if err := session.DB(ctx).QueryRow(ctx, `SELECT COUNT(1) FROM messages WHERE client_id=$1 AND created_at > $2 AND created_at < $3`, l.ClientID, startAt, endAt).Scan(&msgCount); err != nil {
		return err
	}
	if err := session.DB(ctx).QueryRow(ctx, `SELECT COUNT(distinct(user_id)) FROM messages WHERE client_id=$1 AND created_at > $2 AND created_at < $3`, l.ClientID, startAt, endAt).Scan(&userCount); err != nil {
		return err
	}
...
func createLiveReplay(ctx context.Context, r *LiveReplay) error {
	...
	_, err := session.DB(ctx).Exec(ctx, query, r.MessageID, r.ClientID, r.Data, r.Category, r.CreatedAt, r.LiveID)
	return err
}
...
func TopNews(ctx context.Context, u *ClientUser, newsID string, isCancel bool) error {
	...
	if _, err := session.DB(ctx).Exec(ctx, `UPDATE lives SET top_at=$2 WHERE live_id=$1`, newsID, t); err != nil {
		tools.Println(err)
	}
	if _, err := session.DB(ctx).Exec(ctx, `UPDATE broadcast SET top_at=$2 WHERE message_id=$1`, newsID, t); err != nil {
		tools.Println(err)
	}
	return nil
}
...
`, func(rows pgx.Rows) error {
	...
	if _, err := session.DB(ctx).Exec(ctx, durable.InsertQuery("live_play", "live_id,user_id,addr"), liveID, u.UserID, addr); err != nil {
		tools.Println(err)
	}
	return lrs, err
}
...
func sendLiveCard(ctx context.Context, clientID, url, liveID string) error {
	...
	if url == "" {
		...
		if err := session.DB(ctx).QueryRow(ctx, `SELECT data FROM live_replay WHERE live_id=$1`, liveID).Scan(&url); err != nil {
			tools.Println(err)
		}
	} else {
