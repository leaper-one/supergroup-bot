...
func UpdateClientUser(ctx context.Context, user *ClientUser, fullName string) (bool, error) {
	...
	if user.PayExpiredAt.IsZero() {
		...
		_, err = session.DB(ctx).Exec(ctx, query, user.ClientID, user.UserID, user.AuthorizationID, user.Scope, user.PrivateKey, user.Ed25519, user.AccessToken, user.Priority, user.Status)
	} else {
		...
		_, err = session.DB(ctx).Exec(ctx, query, user.ClientID, user.UserID, user.AuthorizationID, user.Scope, user.PrivateKey, user.Ed25519, user.AccessToken, user.Priority, user.Status, ClientUserStatusLarge, user.PayExpiredAt)
	}
	if isNewUser {
...
func CreateOrUpdateClientUser(ctx context.Context, u *ClientUser) error {
	...
	_, err := session.DB(ctx).Exec(ctx, query, u.ClientID, u.UserID, u.AccessToken, u.Priority, u.Status, u.DeliverAt, u.ReadAt)
	return err
}
...
ORDER BY created_at
`, addQuery)
	err := session.DB(ctx).ConnQuery(ctx, query, func(rows pgx.Rows) error {
		for rows.Next() {
			var u ClientUser
...
func updateClientUserStatus(ctx context.Context, clientID, userID string, status int) error {
	_, err := session.DB(ctx).Exec(ctx, `UPDATE client_users SET status=$3 WHERE client_id=$1 AND user_id=$2`, clientID, userID, status)
...
func UpdateClientUserPriority(ctx context.Context, clientID, userID string, priority int) error {
	_, err := session.DB(ctx).Exec(ctx, `UPDATE client_users SET priority=$3 WHERE client_id=$1 AND user_id=$2`, clientID, userID, priority)
	session.Redis(ctx).QDel(ctx, fmt.Sprintf("client_user:%s:%s", clientID, userID))
	return err
}

func UpdateClientUserPriorityAndStatus(ctx context.Context, clientID, userID string, priority, status int) error {
	_, err := session.DB(ctx).Exec(ctx, `UPDATE client_users SET priority=$3,status=$4 WHERE client_id=$1 AND user_id=$2`, clientID, userID, priority, status)
	session.Redis(ctx).QDel(ctx, fmt.Sprintf("client_user:%s:%s", clientID, userID))
	return err
}

func UpdateClientUserActive(ctx context.Context, clientID, userID string, priority, status int) error {
	_, err := session.DB(ctx).Exec(ctx, `UPDATE client_users SET priority=$3,status=$4,deliver_at=$5,read_at=$5 WHERE client_id=$1 AND user_id=$2`, clientID, userID, priority, status, time.Now())
	session.Redis(ctx).QDel(ctx, fmt.Sprintf("client_user:%s:%s", clientID, userID))
	return err
}

func UpdateClientUserPayStatus(ctx context.Context, clientID, userID string, status int, expiredAt time.Time) error {
	_, err := session.DB(ctx).Exec(ctx, `UPDATE client_users SET status=$3,priority=1,pay_status=$3,pay_expired_at=$4 WHERE client_id=$1 AND user_id=$2`, clientID, userID, status, expiredAt)
	session.Redis(ctx).QDel(ctx, fmt.Sprintf("client_user:%s:%s", clientID, userID))
	return err
}
...
func getClientPeopleCount(ctx context.Context, clientID string) (decimal.Decimal, decimal.Decimal, error) {
	...
	if err != nil {
		if errors.Is(err, redis.Nil) {
			if err := session.DB(ctx).QueryRow(ctx, queryAll, clientID, ClientUserStatusExit).Scan(&all); err != nil {
	...
	if err != nil {
		if errors.Is(err, redis.Nil) {
			if err := session.DB(ctx).QueryRow(ctx, queryWeek, clientID, ClientUserStatusExit).Scan(&week); err != nil {
				return decimal.Zero, decimal.Zero, err
			}
...
func GetClientUserStat(ctx context.Context, u *ClientUser) (map[string]int, error) {
	...
	if err := session.DB(ctx).QueryRow(ctx, `SELECT count(1) FROM client_users WHERE client_id=$1`, u.ClientID).Scan(&allUserCount); err != nil {
		return nil, err
	}
	if err := session.DB(ctx).QueryRow(ctx, `SELECT count(1) FROM client_block_user WHERE client_id=$1`, u.ClientID).Scan(&blockUserCount); err != nil {
		return nil, err
	}
	if err := session.DB(ctx).QueryRow(ctx, `SELECT count(1) FROM client_users WHERE client_id=$1 AND muted_at>NOW()`, u.ClientID).Scan(&muteUserCount); err != nil {
		return nil, err
	}
	if err := session.DB(ctx).QueryRow(ctx, `SELECT count(1) FROM client_users WHERE client_id=$1 AND status=8`, u.ClientID).Scan(&guestUserCount); err != nil {
		return nil, err
	}
	if err := session.DB(ctx).QueryRow(ctx, `SELECT count(1) FROM client_users WHERE client_id=$1 AND status=9`, u.ClientID).Scan(&adminUserCount); err != nil {
		return nil, err
	}
