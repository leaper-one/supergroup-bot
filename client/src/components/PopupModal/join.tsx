...
export const JoinModal = (props: Props) => {
  ...
  const { title, desc, buttonAction, button, tips, tipsStyle, tipsAction, isAirdrop, icon, icon_url,
    buttonStyle, descStyle, disabled, loading, content, titleDesc } = props.modalProp
  ...
  return (
    <div className={`${styles.modal} ${isAirdrop ? styles.airdrop : ""}`}>
      ...
      <Button className={styles.button} loading={loading} type={buttonStyle} onClick={buttonAction} disabled={disabled}>  {button}</Button>
      {tips && <span className={`${styles.tips} ${tipsStyle ? styles[tipsStyle] : ""}`} onClick={tipsAction}>{tips}</span>}
    </div>
  )
}
