...
export default () => {
  ...
  const [groupList, setGroupList] = useState<Array<IGroupItem>>([] as IGroupItem[]);
  ...
  useEffect(() => {
    if (search) ApiGetGroupList().then((list) => setGroupList(list.filter((item) => item.name.toLowerCase().includes(search.toLowerCase()))));
    else
  ...
  return (
    <div className={styles.container}>
      <BackHeader name={$t('join.title')} action={mixinCtx && <i onClick={handleClickShared} className={`iconfont iconic_share ${styles.shareIcon}`} />} />
      <div className={styles.search}>
        <Icon i="search" />
        <input type="text" placeholder={$t('join.search.name')} value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
