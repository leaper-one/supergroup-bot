...
import { ApiGetArticle, ApiGetArticleCrawlList, ApiGetArticleCrawlListByID, ApiPutArticle, ApiPutArticleCrawl, ApiPutArticlePass, IArticle } from '@/apis/article';
...
export default () => {
  ...
  const reloadCrawlList = (id = '') => (id ? ApiGetArticleCrawlListByID(id).then(setArticleCrawlList) : ApiGetArticleCrawlList().then(setArticleCrawlList));
  ...
  useEffect(() => {
    ...
    if (s === 0) {
      setComment(`你的文章《${activeArticle.title}》${activeArticle.link} 不符合活动内容要求，奖励已取消。`);
    } else if (s < 1) {
      setComment(`你的文章《${activeArticle.title}》${activeArticle.link} 推荐系统从 1 降为 ${score}，文章质量有待提升，感谢参与。`);
    } else if (s < 2) {
      setComment(`你的文章《${activeArticle.title}》${activeArticle.link} 推荐系统从 1 升为 ${score}，文章不错继续加油！`);
    } else {
      setComment(`你的文章《${activeArticle.title}》${activeArticle.link} 被选为优秀文章，推荐系统从 1 升为 ${score}，同时立刻获得 1 篇投稿资格，你还可以再多投 1 篇文章。`);
    }
  }, [score]);
  ...
  const switchTab = () => {
    switch (activeTab) {
      ...
      case 2:
        return setActiveList(articleList.filter((item) => item.status === '2' && item.score === '1'));
      case 3:
        return setActiveList(articleList.filter((item) => item.status === '2' && item.score !== '1'));
    }
  };
    ...
    <div>
            ...
            {activeList.map((item) => (
              <li key={item.article_id}>
                ...
                <span title={item.key_word}>
                  {item.key_word}
                  <a href={`https://www.google.com/search?q=${encodeURIComponent(item.key_word)}`} target="_blank">
                    <Icon i="google" />
                  </a>
                  <a href={`https://www.baidu.com/s?wd=${encodeURIComponent(item.key_word)}`} target="_blank">
                    <Icon i="baidu1" />
                  </a>
                </span>
                ...
                <span>
                  {item.status === '2' ? (
                    ...
                    <button
                      onClick={async () => {
                        const isConfirm = await Confirm('提示', '确认审核通过吗？');
                        if (isConfirm) {
            ...
            {articleCrawlList.map((item, idx) => (
              <li key={idx}>
                <div>
                  ...
                  <span onClick={() => item.rank && reloadCrawlList()}>{articleMap[item.article_id]?.full_name}</span>
                </div>
                <span title={articleMap[item.article_id]?.key_word}>
                  {articleMap[item.article_id]?.key_word}

                  <a href={`https://www.google.com/search?q=${encodeURIComponent(articleMap[item.article_id]?.key_word)}`} target="_blank">
                    <Icon i="google" />
                  </a>
                  <a href={`https://www.baidu.com/s?wd=${encodeURIComponent(articleMap[item.article_id]?.key_word)}`} target="_blank">
                    <Icon i="baidu1" />
                  </a>
                </span>
                <span title={articleMap[item.article_id]?.title} onClick={() => !item.rank && reloadCrawlList(item.article_id)}>
      ...
      >
        <div className={styles.modal}>
          <label>
            链接：
            <input type="text" placeholder="1" value={link} onChange={(e) => setLink(e.target.value)} />
          </label>
          <p>0：百度 1：谷歌</p>
          <label>
            平台：
            <input type="text" placeholder="1" value={platform} onChange={(e) => setPlatform(e.target.value)} />
          </label>
          <label>
            排名：
            <input type="text" placeholder="1" value={rank} onChange={(e) => setRank(Number(e.target.value))} />
          </label>
      ...
      >
        <div className={styles.modal}>
          ...
          <label>
            备注：
            <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="请输入发送给用户的消息" />
          </label>
        </div>
      </Modal>
    </div>
