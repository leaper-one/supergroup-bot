...
export default function Page() {
  ...
  const clickSetGuestOrManager = async (user: IUser) => {
    ...
    const isConfirm = await Confirm(
      $t('action.tips'),
      $t(`member.action.${user.status === status ? 'confirmCancel' : 'confirmSet'}`, {
        full_name,
        identity_number,
  ...
  useEffect(() => {
    ...
    timer = setTimeout(() => ApiPostSearchUserList(key).then(setSearchList), 500);
  }, [key]);

  return (
    <div className={`${styles.container} safe-view`}>
      <BackHeader name={$t('member.title')} action={<i className={`iconfont iconshaixuan ${styles.filter}`} onClick={() => setStatusModal(true)} />} />
      ...
      >
        {(searchList || userList)?.map((item, idx) => (
          <SwipeAction className={styles.swiper} key={idx} right={getActionList($t, item, isOwner, clickSetGuestOrManager, setMuteModal, clickBlock)}>
...
const MuteModal = (props: IMuteModalProps) => (
  <Modal popup visible={props.muteModal} onClose={() => props.setMuteModal(false)} animationType="slide-up">
    <div className={styles.modal}>
      <h4>{props.$t('member.action.mute')}</h4>
      <img className={styles.modalClose} src={require('@/assets/img/svg/closeBtn.svg')} alt="" onClick={() => props.setMuteModal(false)} />
      <div className={styles.modalInput}>
        <input type="text" value={props.muteTime} onChange={(e) => props.setMuteTime(e.target.value)} />
        <span className={styles.modalUint}>{props.$t('member.modal.unit')}</span>
      </div>
      <p className={styles.modalDesc}>{props.$t('member.modal.desc')}</p>
      <Button onClick={() => props.clickMute()}>{props.$t('action.continue')}</Button>
    </div>
  </Modal>
);
...
const StatusModal = (props: ITypeModalProps) => (
  <Modal popup visible={props.statusModal} onClose={() => props.setStatusModal(false)} animationType="slide-up">
    <div className={styles.modal}>
      <h4>{props.$t('member.status.title')}</h4>
      <img className={styles.modalClose} src={require('@/assets/img/svg/closeBtn.svg')} alt="" onClick={() => props.setStatusModal(false)} />
      <ul className={styles.modalStatusList}>
        {statusList.map((type) => (
          ...
          >
            ...
            <p>
              {props.stat && props.stat[type as TStatus]} {props.$t(`member.status.people`)}
            </p>
            {props.status === type && <i className={`iconfont iconcheck ${styles.modalStatusActive}`}></i>}
          </li>
        ))}
      </ul>
    </div>
