...
export default function Page() {
  ...
  const clickSetGuestOrManager = async (user: IUser) => {
    ...
    const isConfirm = await Confirm(
      $t("action.tips"),
      $t(`member.action.${user.status === status ? 'confirmCancel' : 'confirmSet'}`, { full_name, identity_number, c: $t(`member.action.${isOwner ? 'admin' : 'guest'}`) })
    )
    if (!isConfirm) return
    let is_cancel = false
  ...
  const clickMute = async () => {
    ...
    const isConfirm = await Confirm(
      $t("action.tips"),
      $t("member.action.confirmMute", { full_name, identity_number, mute_time: muteTime })
    )
    if (!isConfirm) return
  ...
  return (
    <div className={`${styles.container} safe-view`}>
      <BackHeader name={$t('member.title')} action={
        <i className={`iconfont iconshaixuan ${styles.filter}`} onClick={() => setStatusModal(true)} />
      } />
      ...
      <div className={styles.list} onScroll={event => {
        ...
        {(searchList || userList)?.map((item, idx) =>
          <SwipeAction
            ...
            key={idx}
            right={getActionList($t, item, isOwner, clickSetGuestOrManager, setMuteModal, clickBlock)}
          >
...
const StatusModal = (props: ITypeModalProps) => <Modal
  popup
  ...
  <div className={styles.modal}>
    ...
    <ul className={styles.modalStatusList}>
      ...
      >
        ...
        <p>{(props.stat && props.stat[type as TStatus])} {props.$t(`member.status.people`)}</p>
...
function getActionList($t: any, item: IUser, isOwner: boolean, clickSetGuestOrManager: any, setMuteModal: any, clickBlock: any): any {
  let actionList = [{
    text: $t(`member.action.${item.status! > 5 ? 'cancel' : 'set'}`, { c: $t(`member.action.${isOwner ? 'admin' : 'guest'}`) }),
