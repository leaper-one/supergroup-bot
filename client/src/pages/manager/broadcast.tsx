      ...
      <div className={styles.list}>
        {broadcastList.map((item, index) => (
          <SwipeAction
            ...
            right={[
              {
                text: $t("manager.broadcast.recall"),
                style: { backgroundColor: "#FA596D", color: "#fff" },
                onPress: async () => {
                  const isConfirm = await Confirm(
                    $t("action.tips"),
                    $t("manager.broadcast.confirmRecall"),
                  )
                  if (isConfirm) {
                    const res = await ApiGetBroadcastRecall(item.broadcast_id)
                    if (res) ToastSuccess($t("manager.broadcast.recallSuccess"))
                    initPage()
                  }
                },
              },
