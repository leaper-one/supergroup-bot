...
export const Broadcast = () => {
  ...
  useEffect(() => {
    ...
    setBroadcastList(tmpData.filter((data) => data.data.toLowerCase().includes(searchKey.toLowerCase())));
  }, [searchKey]);
  ...
  return (
    <div className={styles.container}>
      <BackHeader name={$t('broadcast.a')} action={<i className={styles.addIcon + ' iconfont iconic_add'} onClick={() => history.push(`/broadcast/send`)} />} />
      <div className={styles.input}>
        <img src={require('@/assets/img/svg/search.svg')} alt="" />
        <input value={searchKey} onChange={(e) => setSearchKey(e.target.value)} type="text" placeholder="Keyword" />
      </div>
        ...
        {broadcastList.map((item, index) => (
            ...
            right={[
              {
                ...
                onPress: async () => {
                  const isConfirm = await Confirm($t('action.tips'), $t('broadcast.confirmRecall'));
                  if (isConfirm) {
          ...
          >
            <li className={styles.bItem}>
              ...
              <p>{item.data.length > 100 ? item.data.slice(0, 100) + '...' : item.data}</p>
              <span className={styles[`status` + (item.status || '0')]}>{$t('broadcast.status' + (item.status || '0'))}</span>
              <span className={styles.date}>{moment(item.created_at).format('YYYY/MM/DD HH:mm:ss')}</span>
            </li>
          </SwipeAction>
        ))}
      </div>
