...
const authList = ['plain_sticker', 'lucky_coin', 'plain_image', 'plain_video', 'plain_post', 'plain_data', 'plain_live', 'plain_contact', 'plain_transcript', 'url', 'app_card'];
  ...
  return (
    <div className="safe-view">
      ...
      <ul className={`${styles.list} ${authStyles.list}`}>
        {authList.map((item) => (
          <li key={item} className={styles.formItem}>
            <div className={styles.formItemLeft}>
              <p>{$t('advance.' + item)}</p>
            </div>
            <Switch color="black" checked={auth[item]} onChange={() => setAuth({ ...auth, [item]: !auth[item] })} />
          </li>
        ))}
      </ul>
      <Button
