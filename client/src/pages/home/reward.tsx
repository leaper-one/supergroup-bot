...
export default function Page() {
  ...
  const initPage = async () => {
    ...
    if (!group.asset_id) {
      group.asset_id = group.client_id === '47cdbc9e-e2b9-4d1f-b13e-42fec1d8853d' ? 'c94ac88f-4671-3976-b60a-09064f1811e8' : 'c6d0c728-2624-429b-8e0d-d9d19b6592fa';
    }
  ...
  return (
    <>
      <div className={styles.container}>
        ...
        {activeCoin && (
          <div className={`${styles.coin} ${styles.item}`} onClick={() => setCoinModal(true)}>
        ...
        <PopCoinModal coinModal={coinModal} setCoinModal={setCoinModal} activeCoin={activeCoin} setActiveCoin={setActiveCoin} $t={$t} />

        <PopAdminAndGuestModal activeUser={activeUser} setActiveUser={setActiveUser} userModal={userModal} setUserModal={setUserModal} $t={$t} />
      </div>
...
export const checkPaid = async (amount: string, asset_id: string, counter_user_id: string, trace_id: string, $t: any): Promise<string> =>
  new Promise((resolve) => {
    const check = async () => {
