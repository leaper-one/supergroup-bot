...
export default function () {
  ...
  useEffect(() => {
    ...
    else if (currentStatus === 1) setRecordList(formatRecord(allList.filter((item) => item.status === 1)));
    else if (currentStatus === 2) setRecordList(formatRecord(allList.filter((item) => item.status === 2)));
  }, [currentStatus]);
    ...
    <div>
      <div className={styles.container}>
        ...
        <ul className={styles.tab}>
          {[0, 2, 1].map((status) => (
            <li key={status} className={`${styles.tabItem} ${currentStatus === status && styles.active}`} onClick={() => setCurrentStatus(status)}>
              {' '}
              {$t('mint.record.' + status)}{' '}
            </li>
          ))}
        </ul>
      ...
      {showMask && (
        <div className={styles.mask} onClick={() => setShowMask(false)}>
          <div className={styles.mask_content} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.mask_btn}>
              <div className={styles.btn_item_fx_1} onClick={() => setShowMask(false)}>
                知道了
              </div>
            </div>
          </div>
