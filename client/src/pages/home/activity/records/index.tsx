...
const isValidResult = (r?: GuessResult) => r != undefined && r >= GuessResult.Pending;
...
export default function GuessRecordsPage() {
  ...
  const fetchPageDate = () => {
    Promise.all([ApiGetGuessPageData(id), ApiGetGuessRecord(id)]).then(([g, r]) => {
      setCoin(g.symbol);
      ...
      <div className={styles.content}>
        <div className={styles.tip}>
          <p className={styles.tipContent}>
            {t(inrow > 1 ? 'guess.records.consecutiveplay' : 'guess.records.play')}
            ...
            <div className={styles.description}>{t('guess.records.condition')}</div>
          </p>
        </div>
        ...
        <div className={styles.vs_detail}>
          <span className={styles.num}>{records?.filter((x) => x.result === GuessResult.Win).length || 0}</span>
          <span className={styles.flag}>:</span>
          <span className={styles.num}>{records?.filter((x) => x.result === GuessResult.lose).length || 0}</span>
        </div>
          ...
          {records &&
            records.map((x) => (
              <li className={styles.item} key={x.date}>
                ...
                <span className={styles[x.type]}>{t(`guess.records.${x.type}`)}</span>
