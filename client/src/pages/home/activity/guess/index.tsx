...
const GuessOption: FC<GuessOptionProps> = React.memo(({ label, name, checked, disabled, onChange }) => {
  return (
    <div className={styles.option}>
      <div className={styles[GuessType[name]]} />
      <span className={styles.label}>{label}</span>
      <Radio name={GuessType[name]} onChange={onChange} checked={checked} disabled={disabled} />
    </div>
  );
});
...
type ModalType = 'choose' | 'confirm' | 'success' | 'missing' | 'end' | 'notstart';
...
export default function GuessPage() {
  ...
  const handleSubmitValidate = () => {
    ...
    if (isDateNotStart || (start && start.getTime() >= utcNow.getTime() && isHHmmNotStart) || isHHmmNotStart) {
  ...
  const renderModalBtn = () => {
    if (modalType === 'confirm' || (!modalType && prevModalTypeRef.current === 'confirm')) {
      return (
        <div className={styles.btnGroup}>
          <Button className={styles.btn} onClick={handleSubmit}>
            {t('guess.sure')}
          </Button>
          <Button className={styles.btn} kind="warning" onClick={handleModalClose}>
            {t('guess.notsure')}
          </Button>
        </div>
      );
  ...
  return (
    <div className={styles.container}>
      <BackHeader name={'猜价格赢 ' + coin} isWhite action={<Icon i="ic_file_text" className={styles.record} onClick={navToRecords} />} />
      <div className={styles.header}>
        <h1 className={styles.title}>{t('guess.todayGuess', { coin })} </h1>
        <p className={styles.description}>{t('guess.todyDesc', { coin, usd, time: startTime })}</p>
      </div>
      <div className={styles.content}>
        {/* onChange={handleChange} */}
        <div className={styles.card}>
          <div className={styles.guess}>
            <GuessOption label={t('guess.up')} name={GuessType.Up} checked={choose === 'Up'} disabled={disabled} onChange={handleChange} />
            <GuessOption label={t('guess.down')} name={GuessType.Down} checked={choose === 'Down'} disabled={disabled} onChange={handleChange} />
            <GuessOption label={t('guess.flat')} name={GuessType.Flat} checked={choose === 'Flat'} disabled={disabled} onChange={handleChange} />
          </div>
          <Button className={styles.confirm} disabled={disabled} onClick={handleSubmitValidate}>
            {t('guess.sure')}
          </Button>
        </div>
      ...
      <Modal visible={!!modalType} transparent onClose={handleModalClose}>
        {(modalType || prevModalTypeRef.current) && (
          <div className={styles.modal}>
            <div className={`${styles.emoji} ${styles[(modalType || prevModalTypeRef.current) as string]}`} />
            <p className={styles.tip}>{t(`guess.${modalType || prevModalTypeRef.current}.tip`)}</p>
