...
export default () => {
  ...
  useEffect(() => {
    if (!environment() || !$get('token')) return history.replace(`/join`);
  ...
  return (
    <div className={styles.mainBox}>
          ...
          action={
            <>
              {avatarUrl ? (
                ...
                <i onClick={() => (window.location.href = getAuthUrl())} className={`iconfont iconshouquandenglu ${styles.avatar}`} />
              )}
            </>
          }
        />
      ...
      <div className={styles.statistic}>
        ...
        <div className={styles.content}>
          <div className={styles.content_item}>
            ...
            <span className={`${styles.rate} ${Number(group?.change_usd) > 0 ? styles.green : styles.red}`}>{Number((Number(group?.change_usd) * 100).toFixed(2))}%</span>
          </div>
      ...
      <Modal visible={modal} animationType="slide-up" popup onClose={() => setModal(false)}>
        <JoinModal
          modalProp={{
