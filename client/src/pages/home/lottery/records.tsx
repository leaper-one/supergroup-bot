...
export default function RecordListPage() {
  ...
  const fetchList = (isLottery?: boolean, p: number = 1) => {
    if (isLottery) {
      ...
      return ApiGetLotteryRecord(p, lotteryRecords).then(({ hasMore, list }) => {
  ...
  const renderList = <T extends HTMLDivElement>(data: RecordByDate[], hasMore: boolean = true, ref: LegacyRef<T>, sentryRef: LegacyRef<T>) => (
          ...
          {list.map((r, idx) => (
            <li key={idx} className={styles.record}>
              <div className={styles.recordLeft}>
                <div className={styles.logo}>{r.icon_url ? <img src={r.icon_url} /> : <Icon i={powerTypeMap[r.power_type!]} />}</div>
              ...
              <div className={styles.recordRight}>
                ...
                <span className={styles.desc}>{r.symbol ? r.symbol : $t('claim.energy.title')}</span>
              </div>
            </li>
          ))}
        </ul>
  ...
  const lotteryList = renderList(lotteryRecords, hasMoreLottery, lotteryScrollCtx.rootRef, lotterySentryRef);

  const energyList = renderList(claimRecords, hasMoreClaim, claimScrollCtx.rootRef, claimSentryRef);

  return (
    <>
      <BackHeader name={$t('claim.records.title')} />
      <div className={styles.page}>
        <TabSwitchBar activeRight={isClaimTab} onSwitch={setIsClaimTab} leftLabel={$t('claim.records.winning')} rightLabel={$t('claim.records.energy')} />
        <div className={`${styles.tabPanel} ${isClaimTab ? styles.switch : ''}`}>
...
const TabSwitchBar: FC<TabSwitchBarProps> = ({ activeRight, onSwitch, leftLabel, rightLabel }) => {
  return (
