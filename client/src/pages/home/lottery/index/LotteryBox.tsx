...
export const LotteryBox: FC<LotteryBoxProps> = ({ data, ticketCount = 0, onStart, onEnd, onImgLoad, onPrizeClick }) => {
  ...
  return (
    ...
    >
      ...
      <div className={styles.lottery}>
        {data?.map((reward) => (
          <div key={reward.lottery_id} className={`${styles.reward} ${activeReward === reward.lottery_id ? styles.active : ''}`}>
            <div className={styles.prize} onClick={handlePrizeClick} data-id={reward.lottery_id}>
              ...
              />
              <p className={styles.amount}>{reward.asset_id == 'c6d0c728-2624-429b-8e0d-d9d19b6592fa' ? (Number(reward.amount) * 1e8).toFixed() : reward.amount}</p>
            </div>
          </div>
        ))}
        <div className={styles.content}>
          ...
          <div className={styles.startWrapper}>
            <div className={styles.start}>
              <button disabled={disabled} onClick={handleStartClick} className={(!disabled && ticketCount > 0 && styles.active) || ''}>
                <div>{$t('claim.now')}</div>
