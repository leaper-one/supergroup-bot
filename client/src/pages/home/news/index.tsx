...
export default function Page() {
  ...
  for (let i = 0; i < liveList.length; i++) {
    ...
    else if (Number(new Date(liveList[i].top_at)) > 0)
      top.push({ ...liveList[i], is_top: true })
    else normal.push(liveList[i])
  }
  for (let i = 0; i < broadcastList.length; i++) {
    if (Number(new Date(broadcastList[i].top_at)) > 0)
      top.push({ ...broadcastList[i], is_top: true })
                            ...
                            : [
                              {
                                ...
                                async onPress() {
                                  ...
                                  const isConfirm = await Confirm(
                                    ...
                                    isTop
                                      ? $t("news.confirmTop")
                                      : $t("news.confirmCancelTop"),
                                  )
                                  if (!isConfirm) return
                                  let res
                                  if (isTop) {
                                    res = await ApiGetCancelTopNews(
                                      item.live_id,
                                    )
                                  } else {
...
const NewsTypeActionModal = (props: INewsActionProps) => (
  ...
  >
    <div className={styles.addSelect}>
      <ul>
        {[
          props.$t("news.sendBroadcast"),
          props.$t("news.sendLive"),
