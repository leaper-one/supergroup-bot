...
import { ApiGetCancelTopNews, ApiGetLiveList, ApiGetStartLive, ApiGetStopLive, ApiGetTopNews, ILive } from '@/apis/live';
...
export default function Page() {
  ...
  const initList = async () => {
    ...
    setLiveList([]);
    setBroadcastList([]);
    const [l1, l2] = await Promise.all([ApiGetBroadcastList(), ApiGetLiveList()]);
    setBroadcastList(l1);
    setLiveList(l2);
    setLoaded(true);
  };
  ...
  const topList = top.sort((a, b) => Number(new Date(b.top_at)) - Number(new Date(a.top_at)));
  const lowList = normal.sort((a, b) => Number(new Date(b.created_at!)) - Number(new Date(a.created_at!)));
  if (activeTab === 0) {
    ...
    newsList = broadcastList.sort((a, b) => {
      const hasTop = Number(new Date(b.top_at!)) - Number(new Date(a.top_at));
      if (hasTop !== 0) return hasTop;
      return Number(new Date(b.created_at!)) - Number(new Date(a.created_at!));
    });
    ...
    <>
      {loaded ? (
        <div className="safe-view">
          <BackHeader name={$t('home.article')} action={isManager ? <i className={`iconfont iconic_add`} onClick={() => setShow(true)} /> : <></>} />
              ...
              {newsList.map((item: any, idx: number) => (
                <div key={idx}>
                  {item.img_url ? (
                    ...
                    >
                      <div
                        className={`${styles.item} ${styles.live}`}
                        onClick={() => {
                          $set('active_live', item);
                      ...
                      <div className={`${styles.broadcast} ${styles.item}`}>
                        <p
                          dangerouslySetInnerHTML={{
                            __html: handleBroadcast(item.data),
                          }}
                        />
                        <div className={styles.footer}>
                          <div className={styles.footer_l}>
                            {Number(new Date(item.top_at)) > 0 && <Icon i="ic_notice" />}
