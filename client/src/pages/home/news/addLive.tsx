  ...
  return <div>
    ...
    <div className={styles.content}>
      <div className={styles.img}>
        {form.img_url ? <img className={styles.imgView} src={form.img_url} alt=""/> : <>
    ...
    <NewsCategorySelectModal
      show={show}
      setShow={setShow}
      $t={$t}
      category={form.category}
      setCategory={(v) => setForm({ ...form, category: v })}/>
  </div>
}
...
const NewsCategorySelectModal = (props: INewsFormCategory) => <Modal
  popup
...
>
  <div className={styles.categorySelect}>
    <i className={`iconfont iconguanbi ${styles.close}`} onClick={() => props.setShow(false)}/>
    ...
    <ul>
      {[1, 2].map((item) => (
        <li key={item} onClick={() => {
          ...
          {item === props.category && (
            <i className={`iconfont iconcheck`}/>
          )}
        </li>
      ))}
    </ul>
