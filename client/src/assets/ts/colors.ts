...
function uuidHashCode(id: string) {
...
function ios(id: string) {
  let components = id.split('-');
  let mostSigBits: any = parseInt(components[0], 16);
  mostSigBits = new BigNumber(mostSigBits);
  mostSigBits = mostSigBits.multipliedBy(new BigNumber(2).pow(16)).toString();
  mostSigBits = asIntN(64, mostSigBits);
  let c1 = parseInt(components[1], 16);
  mostSigBits = bigIntOr(mostSigBits, c1);
  mostSigBits = new BigNumber(mostSigBits).multipliedBy(new BigNumber(2).pow(16)).toString();
  mostSigBits = asIntN(64, mostSigBits);
  let c2 = parseInt(components[2], 16);
  mostSigBits = bigIntOr(mostSigBits, c2);
  mostSigBits = asIntN(64, mostSigBits);
  let leastSigBits: any = parseInt(components[3], 16);
  leastSigBits = new BigNumber(leastSigBits).multipliedBy(new BigNumber(2).pow(48)).toString();
  ...
  let m = new BigNumber(hilo).div(new BigNumber(2).pow(32)).toString();
  let n = asIntN(32, hilo);
  let result: any = bigIntXOr(m, n);
  result = asIntN(32, result);
  return Math.abs(result);
}

function asIntN(bits: number, number: number | string): string {
  let binNum = toBin(number);
  let { length } = binNum;
  if (length < bits) return new BigNumber(binNum, 2).toString();
  let t = binNum.substr(binNum.length - bits);
  if (t[0] === '0') {
    return new BigNumber(t, 2).toString();
  } else if (t[0] === '1') {
    t = t.substr(1);
    let result = '';
    ...
    return new BigNumber(result, 2).plus(1).multipliedBy(-1).toString();
  }
  return '';
}

function toBin(intNum: any, bits = 64) {
  if (/\d+/.test(intNum)) {
    BigNumber.config({ ROUNDING_MODE: 3, DECIMAL_PLACES: 0 });
    intNum = new BigNumber(intNum);
...
function bigIntOr(str1: string, str2: string | number, bits = 64): string {
  let result = '';
  ...
  return new BigNumber(result, 2).toString();
}

function bigIntXOr(str1: string, str2: string | number, bits = 64): string {
  let result = '';
  str1 = toBin(str1, bits);
  str2 = toBin(str2, bits);
  if (str1.length < str2.length) {
