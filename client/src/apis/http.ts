...
export const getAuthUrl = ({ returnTo = '', hasAssets = false, hasSnapshots = false, state = '' } = {}) => {
  ...
  if (!returnTo) {
    returnTo = pathname === '/auth' ? (query?.return_to as string) || '/' : pathname + search;
  }
...
export const payUrl = ({ trace = '', recipient = getClientID(), asset = '', amount = '', memo = '' } = {}) =>
  `https://mixin.one/pay?recipient=${recipient}&asset=${asset}&amount=${amount}&memo=${encodeURIComponent(memo)}&trace=${trace}`;
...
export const getExinSwapUrl = (id: string) => `https://app.exinswap.com/#/pairs/${id}`;
...
export const apis = {
  get: (url: string, params: any = {}, options: any = {}): Promise<any> => request(url, { params, ...options }),
  post: (url: string, data: any = {}, options: any = {}): Promise<any> => request(url, { method: 'POST', data, ...options }),
  put: (url: string, data: any = {}): Promise<any> => request(url, { method: 'PUT', data }),
  delete: (url: string, params: any = {}, options: any = {}): Promise<any> => request(url, { method: 'DELETE', params, ...options }),
};
