...
export const ApiGetAssetByID = (id: string): Promise<IAsset> =>
  apis.get(`${mixinBaseURL}/network/assets/${id}`)

export const ApiGetAssetBySymbol = async (
  symbol: string,
): Promise<IAsset[]> => {
  if (!GlobalData.AssetSymbol) GlobalData.AssetSymbol = {}
  if (!GlobalData.AssetSymbol[symbol])
    GlobalData.AssetSymbol[symbol] = await apis.get(
      `${mixinBaseURL}/network/assets/search/${symbol}`,
    )
...
export const ApiGetMyAssets = async (): Promise<IAsset[]> => {
  if (!GlobalData.MyAssetList) {
    ...
    GlobalData.MyAssetList = assetList
...
export const ApiCheckIsPaid = (
  params: ICheckPaidParams,
): Promise<ICheckPaidResp> => apis.post(`${mixinBaseURL}/payments`, params)
